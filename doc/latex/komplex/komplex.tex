...
Politische Analyse krankt an einer strukturellen Unschärfe. Begriffe wie „links“, „rechts“, „autoritär“ oder „libertär“ werden im Diskurs ständig vermischt, überlagert und je nach Kontext unterschiedlich belegt. Ein und derselbe politische Akteur erscheint verschiedenen Beobachtern an völlig unterschiedlichen Positionen im Meinungsspektrum – nicht notwendigerweise aufgrund unterschiedlicher Faktenkenntnis, sondern aufgrund unterschiedlicher Koordinatensysteme der Bewertung.

Dieser Text unternimmt den Versuch, dieser Unschärfe mit den Mitteln der Mathematik zu begegnen. Er definiert einen zweidimensionalen politischen Raum, aufgespannt durch die Achsen:

\begin{itemize}
    \item \textbf{Realteil \(a\):} \emph{Staatlichkeit} – von starker Institutionalisierung (\(a > 0\)) bis zu staatsferner Anarchie (\(a < 0\)).
    \item \textbf{Imaginärteil \(b\):} \emph{Ideologische Position} auf der Achse Kommunismus (\(b > 0\), Egalität) – Faschismus (\(b < 0\), Hierarchie).
\end{itemize}

Eine politische Position wird damit als komplexe Zahl \(z = a + ib\) notiert. Diese Wahl ist nicht metaphorisch, sondern strukturell: Sie erlaubt es, Betrag (politische Intensität), Argument (Mischungsverhältnis) und vor allem die Operation der \textbf{Kontext-Rotation} \(C_\varphi(z) = e^{i\varphi} z\) mathematisch exakt zu fassen.

\subsection*{Der Erkenntnisgewinn}

Das Modell erhebt nicht den Anspruch, politische Wahrheiten \emph{vorzuschreiben}. Es beansprucht jedoch, eine \textbf{invariante Geometrie} des politischen Raums sichtbar zu machen, die dem tagespolitischen Streit zugrunde liegt. Insbesondere drei Aspekte treten hervor:

\begin{enumerate}
    \item \textbf{Trennung der Dimensionen:} Staatlichkeit und ideologischer Inhalt werden als orthogonale Größen behandelt. Ein System kann „viel Staat“ und „viel Kommunismus“ sein (z.\,B. \(1+i\)) oder „wenig Staat“ und „viel Faschismus“ (z.\,B. \(-1-i\)). Diese Kombinationen sind in der eindimensionalen Skala nicht abbildbar.
    \item \textbf{Formalisierung von Perspektive:} Der Operator \(C_\varphi\) modelliert, was im öffentlichen Diskurs „Framing“ heißt. Er verändert nicht den Zustand \(z\), sondern das Koordinatensystem des Beobachters. Die mathematische Struktur erklärt, warum zwei Beobachter dasselbe Objekt strukturell unterschiedlich lokalisieren \emph{müssen} – und warum Debatten oft ohne gemeinsamen Referenzrahmen geführt werden.
    \item \textbf{Geometrische Definition von Extremismus:} Die Lage der Idealtypen im Raum erzwingt eine natürliche Grenze zwischen gemäßigten und extremen Positionen. Diese Grenze ist kein willkürlicher Schwellwert, sondern eine Folge der inneren Metrik des Raumes selbst.
\end{enumerate}

\subsection*{Zur Lektüre}

Der erste Teil (Kapitel 1--5) etabliert den statischen Raum und ordnet exemplarisch Parteien der Bundesrepublik Deutschland sowie Referenzideologien ein. Der zweite Teil (Kapitel B) erweitert das Modell zu einer \textbf{Feldtheorie des Politischen}, in der Operatoren für Egalitarismus, Hierarchie und Ordnung die Dynamik von Systemen beschreiben.

Der Text setzt Vertrautheit mit der komplexen Zahlenebene voraus, nicht jedoch mit politikwissenschaftlicher Theoriebildung. Er ist eine Einladung, politische Phänomene mit einer neuen, präziseren Sprache zu beschreiben – nicht um sie zu vereinfachen, sondern um ihrer Komplexität gerecht zu werden.

\vspace{1cm}
\hfill \textit{Michael Czybor, \today}
...
\begin{axis}[
    ...
    xlabel style={anchor=west, font=\small},
    ylabel style={anchor=south, font=\small},
    legend style={at={(0.98,0.02)}, anchor=south east, font=\small, fill=white, fill opacity=0.8}
]
...
% Legende für Achsenrichtungen
\addlegendimage{empty legend}
\addlegendentry{\textbf{\(a>0\): starker Staat | \(a<0\): schwacher Staat}}
\addlegendimage{empty legend}
\addlegendentry{\textbf{\(b>0\): Kommunismus | \(b<0\): Faschismus}}
...
% Legende separat unter der Grafik
\begin{center}
...
\vspace{0.3cm}
\begin{tabular}{ll}
\textcolor{red!70!black}{\textbf{\(\boldsymbol{\rightarrow}\)}} & Egalitarismus \(E\) \(\rightarrow\) Kommunismus (\(+b\)) \\[6pt]
\textcolor{orange!90!black}{\textbf{\(\boldsymbol{\rightarrow}\)}} & Hierarchie \(H\) \(\rightarrow\) Faschismus (\(-b\)) \\[6pt]
\textcolor{teal!70!black}{\textbf{\(\boldsymbol{\rightarrow}\)}} & Ordnung \(O\) \(\rightarrow\) mehr Staatlichkeit (\(+a\)) \\[6pt]
\textcolor{teal!50!black}{\textbf{\(\boldsymbol{-\!\!-\!\!\rightarrow}\)}} & Deregulierung \(\rightarrow\) weniger Staatlichkeit (\(-a\))
\end{tabular}
\vspace{0.3cm}
\end{minipage}}
\end{center}
