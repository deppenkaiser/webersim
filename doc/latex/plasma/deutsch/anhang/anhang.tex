...
Die Analyse der Kollapsdynamik erfordert die numerische Integration der Bewegungsgleichung (Gl. \refeq{eq:bewegungsgleichung}). Mit dem Substitutionsansatz $R(t) = R_0 f(t)$ ergibt sich das folgende
...
Somit ist $Q$ nicht der \textit{Antrieb} des Kollapses, sondern sein fundamentaler \textit{Garant}. Es ist die physikalische Instanz, die die fraktale Strukturbildung (Gl. \refeq{eq:dichtefluktuation})
...
Die Kontinuitätsgleichung, Ausdruck der Ladungserhaltung, ist fundamental in der Struktur der \gls{wdbt} verankert. Ausgangspunkt ist die Erhaltung der Wahrscheinlichkeitsdichte (Gl. \refeq{eq:B2}):
...
Der Coulomb-Anteil der Weber-Kraftdichte (Gl. \refeq{eq:weber_kraftdichte}) führt im Lokalitätslimes ($g(\vec{r}) \rightarrow \delta^{(3)}(\vec{r})$)
...
nicht-lokale Version (Gl. \refeq{eq:modified_ampere}):
