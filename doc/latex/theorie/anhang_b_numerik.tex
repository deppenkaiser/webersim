...
\begin{enumerate}
    ...
    \item \textbf{Vorhersagegenerierung:} Berechnung testbarer experimenteller Konsequenzen (siehe Kapitel~\ref{ch:vorhersagen})
\end{enumerate}

Alle Simulationen basieren auf den in Teil I entwickelten fundamental diskreten Gleichungen (siehe Kapitel~\ref{ch:diskrete_dynamik} und \ref{ch:lagrange}).

\section{Das diskrete Simulationsnetzwerk}
\label{app:numerik_netzwerk}

\subsection{Grundstruktur}
\label{app:numerik_grundstruktur}
...
\label{app:numerik_initialisierung}
...
\label{app:numerik_weber}

Die lokale Dynamik wird durch die diskrete Weber-Kraft implementiert (siehe Kapitel~\ref{ch:dstt_weber}):
...
\label{app:numerik_raum}

Die physikalische Raumstruktur wird aus der Metrik rekonstruiert (siehe Kapitel~\ref{ch:emergenz}):
...
\label{app:numerik_erhaltung}

Die Simulation überprüft kontinuierlich:

\noindent
\textbf{Informationserhaltung:} (siehe Kapitel~\ref{ch:axiome})
...
\begin{itemize}
    ...
    \item \textbf{Vorhersage:} Spezifische nicht-gau{\ss}sche Fluktuationen (siehe Kapitel~\ref{ch:vorhersagen})
\end{itemize}
...
\label{app:numerik_visualisierung}
