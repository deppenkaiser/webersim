...
\label{app:neutrino_q}

\section{Einleitung}
\label{app:neutrino_q_einleitung}
...
\label{app:neutrino_q_definition}

\subsection{Das Quantenpotential in der DBT}
\label{app:neutrino_q_quantenpotential}

In der De-Broglie-Bohm-Theorie lautet das Quantenpotential (Kapitel~\ref{ch:lagrange}, Abschnitt 4.6.2):
...
\label{app:neutrino_q_identifikation}
...
siehe Kapitel~\ref{ch:dstt_weber}), vermittelt das Neutrino die nichtlokale Organisation 
des Informationsfeldes.

\section{Die Neutrinomasse aus der fundamentalen Länge}
\label{app:neutrino_q_masse}

\subsection{Fundamentale Längenskala $l_0$}
\label{app:neutrino_q_l0}
...
\label{app:neutrino_q_masse_formel}
...
Dies ist eine testbare Vorhersage (siehe Kapitel~\ref{ch:vorhersagen}).

\section{Das Neutrino als Vermittler der Nichtlokalität}
\label{app:neutrino_q_nichtlokalitaet}
...
\label{app:neutrino_q_generationen}

\subsection{Moden des fraktalen Q-Feldes}
\label{app:neutrino_q_moden}
...
\label{app:neutrino_q_massenunterschiede}
...
\label{app:neutrino_q_oszillationen}

\subsection{Dispersionsrelation für das Q-Feld}
\label{app:neutrino_q_dispersion}
...
\label{app:neutrino_q_phasengeschw}
...
\label{app:neutrino_q_mesoskala}

Die mesoskopische Skala $l_*$ ist die Korrelationslänge des fraktalen Vakuums.
Sie folgt aus der Vakuumenergiedichte $\rho_{\text{vac}}$ (Anhang~\ref{app:energiebilanz}):

\[
l_* = \left( \frac{\hbar c}{G \rho_{\text{vac}}} \right)^{1/4} \qquad (J.15)
\]

Mit $\rho_{\text{vac}} \sim 10^{-9}\,\text{J/m}^3$ (aus Anhang~\ref{app:energiebilanz} für makroskopische Skalen)
...
\label{app:neutrino_q_vorhersage}
...
(JUNO, DUNE, Hyper-Kamiokande) testbar (siehe Kapitel~\ref{ch:vorhersagen}).

\section{Neutrinos als Dunkle Materie}
\label{app:neutrino_q_dunkle_materie}

\subsection{Kosmologische Neutrinodichte}
\label{app:neutrino_q_dichte}
...
\label{app:neutrino_q_identitaet_dm}
...
\label{app:neutrino_q_zusammenfassung}
