...
Mit \( D \approx 2,704 \) ist \( 2 - D/3 \approx 0,0987 \). Also:

\[
R(M) = K_R \cdot M^{-1/(2 - D/3)} = K_R \cdot M^{-10,13} \tag{L.14}
\]
...
\[
K_R = \left( \frac{2D A \hbar^2}{3(D-2) C_D G m_B^2} \right)^{1/(2 - D/3)} \tag{L.15}
\]

Einsetzen der Werte ergibt:

\[
K_R \approx 5,7 \times 10^{13} \, \text{m} \cdot M_{\odot}^{10,13} \tag{L.16}
\]
...
\[
M_{\text{BEC}} = \left( \frac{K_R}{l_0} \right)^{1/10,13} \tag{L.17}
\]

Mit \( K_R \approx 5,7 \times 10^{13} \, \text{m} \cdot M_{\odot}^{10,13} \) 
und \( l_0 \approx 1,8 \times 10^{-15} \, \text{m} \):

\[
\frac{K_R}{l_0} \approx 3,2 \times 10^{28} M_{\odot}^{10,13}
\]

Dann:

\[
M_{\text{BEC}} \approx \left( 3,2 \times 10^{28} \right)^{1/10,13} M_{\odot}
\]

Wegen \( 1/10,13 \approx 0,0987 \):

\[
M_{\text{BEC}} \approx 10^{28 \cdot 0,0987} \cdot 3,2^{0,0987} M_{\odot}
\]
\[
28 \cdot 0,0987 = 2,76
\]
...
\]
\[
3,2^{0,0987} \approx 1,12
\]
\[
M_{\text{BEC}} \approx 575 \cdot 1,12 \, M_{\odot} \approx 644 M_{\odot}
\]

Dies ist noch nicht der endgültige Wert, da die effektive Konstante 
\( C_D^{\text{eff}} \) aus der selbstkonsistenten Dichteverteilung 
die Skalierung modifiziert. Die vollständige numerische Rechnung 
(siehe Kapitel 10.6) liefert:
...
sind Systeme aus einem zentralen BEC-Objekt (\( M = M_{\text{BEC}} \)) 
und einer massiven Umgebung.
...
BEC-Objekte können verschmelzen, abprallen, zerreißen oder 
Doppelkern-Systeme bilden. Gravitationswellen von BEC-Kollisionen 
unterscheiden sich geringfügig von denen Schwarzer Löcher.

\section{Zusammenfassung}

\begin{itemize}
    \item Minimale Größe: \( l_0 \approx 1,8 \times 10^{-15} \, \text{m} \)
    \item Radius-Masse-Beziehung: \( R \propto M^{-10,13} \)
