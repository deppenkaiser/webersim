% ============================================================================
% KAPITEL 8: Dynamische Schwere-Trägheits-Theorie (DSTT) und Weber-Kräfte
% ============================================================================

\chapter{Dynamische Schwere-Trägheits-Theorie (DSTT) und Weber-Kräfte}
\label{ch:dstt_weber_kraefte}
...
\label{sub:wdbt_kraft_vollstaendig}

Die reine Weber-Gravitation (WG) ist für sich genommen unvollständig. Sie produziert in zweiter Ordnung der Störungsrechnung einen störenden Term \(\alpha\phi^2\), der zu nicht-geschlossenen Bahnen führen würde (siehe Anhang~\ref{app:identitaetsbeziehung}). Die vollständige Theorie ist die \textbf{Weber-De-Broglie-Bohm-Theorie (WDBT)}. In ihr wird die Kraft um das Quantenpotential \(Q\) der De-Broglie-Bohm-Theorie erweitert.
...
Die radiale Komponente des Quantenpotentials kompensiert die störenden Terme der reinen WG und stellt die globale Konsistenz der Theorie sicher (siehe Anhang~\ref{app:identitaetsbeziehung} für den vollständigen Beweis).
...
\label{sub:weber_periheldrehung}
...
Die DSTT-Zustandsvariable \(\beta_{\text{DSTT}}\) und die Terme der \textbf{vollständigen WDBT-Kraft} sind über eine Identitätsbeziehung verknüpft (siehe auch Anhang~\ref{app:identitaetsbeziehung}):
...
Dieser Term wird jedoch durch das Quantenpotential \(Q\) \textbf{exakt kompensiert} (siehe Anhang~\ref{app:identitaetsbeziehung}). Daher gilt die exakte Identität für die vollständige WDBT-Kraft \(\vec{F}_{\text{WDBT}}\).
...
mit \(\vec{F}_{\text{WDBT}} = \vec{F}_{\text{WG}} + \vec{F}_Q\). Die reine WG allein erfüllt diese Beziehung nur näherungsweise; die exakte Gültigkeit wird durch das Quantenpotential \(Q\) hergestellt (siehe Anhang~\ref{app:identitaetsbeziehung}).
...
\label{sec:exzentrizitaetsaenderung}
...
\label{sec:kreisbahnen_unmoeglich}
...
\label{sec:erweiterte_weber_theorie}
...
Die beiden \(\beta\) sind durch die Identitätsbeziehung verknüpft (siehe Anhang~\ref{app:identitaetsbeziehung}):
