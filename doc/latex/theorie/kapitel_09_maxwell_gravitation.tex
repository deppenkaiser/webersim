...
Damit sind die beobachteten Gravitationswellen (z. B. GW170817) vollständig erklärbar – ohne dass die \emph{Kraft selbst} retardiert sein müsste.
...
Im Grenzfall verschwindender Korrekturen (\( v/c \to 0 \)) gehen sie in die bekannten Maxwell-Gleichungen der Gravitation über. Diese Gleichungen sind \textbf{glatt} – sie enthalten nur gewöhnliche Ableitungen (siehe auch Kapitel~\ref{ch:maxwell_fraktale_quellen} für die Erweiterung auf fraktale Quellen).

\section{Die Rolle der DSTT als Vermittler}
\label{sec:dstt_vermittler}
...
beschreibt die Aufteilung der Energie in radiale und tangentiale Komponenten (siehe Kapitel~\ref{ch:dstt_weber}). Diese Aufteilung ist direkt mit der Wirbeldichte verknüpft:
...
\label{sec:omega_feldgleichungen}
...
\label{sec:omega_vorhersagen}

Die Theorie macht spezifische, von der ART abweichende Vorhersagen (siehe auch Kapitel~\ref{ch:vorhersagen}):
...
\begin{itemize}
    ...
    \item Die \textbf{Raumzeit-Metrik} der ART ist emergent – sie entsteht aus der fraktalen Informationsstruktur der IWT (Teil I, siehe Kapitel~\ref{ch:emergenz} und Anhang~\ref{app:kontinuumslimes}).
\end{itemize}
...
\begin{itemize}
    ...
    \item Newtons Gravitationsgesetz emergiert als statischer Grenzfall. Die Periheldrehung des Merkur ist bereits in instantanen Theorien (Weber, Tisserand) enthalten – kein Privileg der ART.
