% ============================================================================
% KAPITEL 10: Bose-Einstein-Kondensat (BEC)-Objekte
% ============================================================================

\chapter{Bose-Einstein-Kondensat (BEC)-Objekte}
\label{ch:bec_objekte}

\section{Einleitung}
\label{sec:bec_einleitung}
...
Die beobachteten supermassereichen Objekte in Galaxienzentren (bis \( 10^{10} M_\odot \)) sind keine einzelnen kompakten Objekte, sondern Systeme aus einem zentralen BEC-Objekt und einer massiven Umgebung (Akkretionsscheibe, Sternhaufen, Gas). Diese Umgebung entsteht durch kontinuierliche Materieentstehung im Quantenpotential des Kerns und die anschließende Drift nach außen (siehe Kapitel~\ref{ch:kosmologie}).
...
wobei \( \lambda_p \) die Compton-Wellenlänge des Protons ist. Diese Länge ist die kleinste physikalisch mögliche Distanz – nichts kann kleiner sein. Sie ersetzt die Planck-Skala als natürliche untere Grenze (siehe auch Anhang~\ref{app:bec}).

\section{Paarbildung und Bose-Einstein-Kondensation}
\label{sec:bec_paarbildung}
...
\begin{itemize}
    ...
    \item Das Quantenpotential dominiert die repulsive Wechselwirkung (siehe Kapitel~\ref{ch:neutrino})
    \item Das Objekt kann bis zur fundamentalen Länge \( l_0 \) schrumpfen
\end{itemize}

\section{Zustandsgleichung eines BEC-Objekts}
\label{sec:bec_zustandsgleichung}
...
Dies ist die maximale Masse eines einzelnen BEC-Objekts. Oberhalb dieser Grenze wäre \( R < l_0 \), was physikalisch unmöglich ist (siehe auch Kapitel~\ref{ch:testbare_vorhersagen}).

\section{Relativistische Korrekturen aus der Weber-Gravitation}
\label{sec:bec_relativistik}

Die Weber-Gravitation (siehe Kapitel~\ref{ch:dstt_weber_kraefte}) führt zu einem modifizierten innersten stabilen Orbit (ISCO). Für eine kreisförmige Bahn (\( \dot{r} = 0, \ddot{r} = -v^2/r \)) liefert die Weber-Gravitation mit \( \beta = 0,5 \):
...
Diese Umgebung entsteht durch die kontinuierliche Materieentstehung aus dem Quantenpotential des Kerns und die anschließende Drift nach außen (siehe Kapitel~\ref{ch:kosmologie} und Anhang~\ref{app:materieentstehung}).

\section{Strukturelle Analogie zum Atom}
\label{sec:bec_atom_analogie}
...
Die fraktale Raumdimension \( D \approx 2,704 \) verbindet beide Welten – sie bestimmt sowohl die Feinstrukturkonstante als auch das Massenverhältnis \( M_{\text{Gal}}/M_{\text{Kern}} \approx 1000 \) (siehe Kapitel~\ref{ch:kosmologie}).

\section{Kollisionen von BEC-Objekten}
\label{sec:bec_kollisionen}
...
\label{sec:bec_gravitationswellen}

Verschmelzungen von BEC-Objekten erzeugen Gravitationswellen – ähnlich wie Neutronensterne oder Schwarze Löcher in der ART. Unterschiede (siehe auch Kapitel~\ref{ch:testbare_vorhersagen}):
...
Die LIGO/Virgo-Daten (\cite{LIGO2016}, \cite{LIGO2017}) sind mit beiden Modellen konsistent, da die beobachteten Massen (\( \sim 10 - 100 M_\odot \)) weit unter \( M_{\text{BEC}} \) liegen.

\section{Zusammenfassung}
\label{sec:bec_zusammenfassung}
