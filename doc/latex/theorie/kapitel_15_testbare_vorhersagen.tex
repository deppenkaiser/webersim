...
\begin{array}{|l|l|l|}
\hline
\text{Vorhersage} & \text{Abweichung von ART} & \text{Experiment} \\
\hline
\text{Lichtablenkung} & \Delta\phi = \Delta\phi_{\text{ART}}(1 + \alpha \lambda^2) & \text{EHT, VLBI, LISA} \\
\text{Gravitationswellen-Dispersion} & v_{\text{phase}} = c\left(1 + \beta \left(\frac{f}{f_0}\right)^{-0,296}\right) & \text{LISA, Einstein-Teleskop} \\
\text{Neutrinomasse} & m_\nu \approx 0,1 \, \text{eV}/c^2 & \text{KATRIN, JUNO, DUNE} \\
\text{Hubble-Skalenabhängigkeit} & H_{\text{eff}}(d) = H_0 (d/d_0)^{-0,296} & \text{Supernovae, Quasare, GW} \\
\text{Maximale kompakte Masse} & M_{\text{BEC}} \approx 3 \times 10^6 M_\odot & \text{Galaxienzentren, EHT} \\
\text{Galaxien-Dichteprofil} & \rho(r) \propto r^{-0,296} & \text{EUCLID, Rubin (LSST)} \\
\text{Rotationskurven} & v(r) \propto r^{0,352} \text{ (asymptotisch steigend)} & \text{Extrem breite Galaxien} \\
\hline
\end{array}
\]
