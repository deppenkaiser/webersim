...
\begin{itemize}
    \item \textbf{Galaxien-Dichteprofil:} \( \rho(r) \propto r^{-(3-D)} \approx r^{-0,296} \) (flacher als das NFW-Profil)
    \item \textbf{Rotationskurven:} \( v(r) \propto r^{(D-2)/2} \approx r^{0,352} \) (bei sehr großen Radien steigt die Kurve langsam an, fällt nicht ab)
    \item \textbf{CMB-Korrelationen:} \( C(\theta) \propto \theta^{-(3-D)} \approx \theta^{-0,296} \) mit einer festen, aus der Dodekaeder-Symmetrie folgenden Achse („Achse des Bösen“)
    \item \textbf{Materieentstehung:} \( \frac{dM_{\text{cre}}}{d\ln r} \propto r^{3-D} \approx r^{0,296} \)
\end{itemize}
...
\hline
Maximale kompakte Masse & \(M_{\text{BEC}} \approx 3\times10^6 M_\odot\) & Galaxienzentren, EHT \\
\hline
...
Die benötigte relative Genauigkeit liegt für die meisten Experimente im Bereich 
\(10^{-6}\) bis \(10^{-2}\). Jede dieser Vorhersagen ist prinzipiell falsifizierbar.
...
\text{EHT-Schatten} & r_{\text{ph}} = 3GM/c^2, \text{aber andere Feinstruktur} & \text{EHT, nächste Generation} \\
\hline
\end{array}
\]
