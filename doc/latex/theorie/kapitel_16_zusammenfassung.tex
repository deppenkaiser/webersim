...
\begin{enumerate}
    \item \textbf{Die Weber-Kraft} – eine geschwindigkeits- und beschleunigungsabhängige direkte Teilchenwechselwirkung, die Elektrodynamik (\( \beta = 2 \)) und Gravitation (\( \beta = 0,5 \) für Massen, \( \beta = 1 \) für Photonen) auf einheitliche Weise beschreibt – ohne Felder und ohne Raumzeitkrümmung (siehe Kapitel~\ref{ch:dstt_weber}).
...
Die DSTT ist eine eigenständige Theorie der Bewegung unter dem Einfluss einer Zentralkraft (siehe Kapitel~\ref{ch:dstt_weber}). Ihre Kernaussagen sind:
...
Im fraktalen Raum lauten die Maxwell-Gleichungen für Elektrodynamik und Gravitation (siehe Kapitel~\ref{ch:maxwell_fraktale_quellen}):
...
\begin{itemize}
    ...
    \item Oszillationen aus fraktaler Dispersion (siehe Anhang~\ref{app:neutrino_q})
...
Die Theorie macht spezifische Vorhersagen, die von der ART und dem Standardmodell abweichen (siehe Kapitel~\ref{ch:vorhersagen}):
