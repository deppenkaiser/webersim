...
die auf Geschwindigkeits- und Beschleunigungstermen basiert, erweitert die \gls{dbt} die \gls{qm} um wohldefinierte Teilchentrajektorien, die durch ein Quantenpotential gesteuert
...
Die \gls{wdbt} wird im Text auch als WG-DBT-Synthese bezeichnet.
...
\subsection{Herleitung der Rotationskurven in der WG-DBT-Synthese}
...
\item $Q = -\frac{\hbar^2}{2m} \frac{\nabla^2 |\Psi|}{|\Psi|}$ das Quantenpotential der \gls{dbt} darstellt.
\end{itemize}

Für Photonen ($m \to 0$) dominiert der \gls{wg}-Term, da $Q \propto 1/m$ divergiert. Die effektive Kraft reduziert sich auf:
...
wobei $E = h_{\text{P}} \nu$ die Photonenenergie ist. Diese Gleichung verallgemeinert die Standardform der \gls{art} um einen wellenlängenabhängigen Term.
...
mit $\lambda_0 = hc/E$ als charakteristischer Längenskala. Der zweite Term repräsentiert die wellenlängenabhängige Korrektur der \gls{wg}-\gls{dbt}-Synthese.
...
Der Shapiro-Effekt beschreibt die gravitative Laufzeitverzögerung elektromagnetischer Signale. Wir leiten ihn hier streng aus der \gls{wg} her und zeigen die Abweichungen von der \gls{art}.

\subsection{Metrik und Nullgeodäten}
In der \gls{wg} ersetzen wir die gekrümmte Raumzeit der \gls{art} durch das Potential:
\begin{equation}
...
\begin{equation}
\Delta t \approx \underbrace{\frac{r_2 - r_1}{c}}_{\text{Newtonsch}} + \underbrace{\frac{2GM}{c^3}\ln\left(\frac{4r_1r_2}{b^2}\right)}_{\text{\gls{art}-Term}} + \underbrace{\frac{3\pi G^2M^2}{4c^5b^2}\left(\frac{v_0^2}{c^2}\right)}_{\text{WG-Korrektur}}
\end{equation}

\subsection{Wellenlängenabhängigkeit}
Die \gls{wg} sagt eine Frequenzabhängigkeit voraus:
\begin{equation}
\frac{\Delta t_{\text{\gls{wg}}}}{\Delta t_{\text{\gls{art}}}} = 1 + \frac{3\pi}{16}\frac{\lambda^2}{\lambda_0^2}
\end{equation}
...
\item Die \gls{art} vernachlässigt den $\lambda$-abhängigen Term vollständig
\end{itemize}

Dies zeigt, dass die \gls{wg} bei hohen Präzisionstests von der \gls{art} abweicht, ohne auf Raumzeitkrümmung zurückzugreifen.
...
\begin{itemize}  
\item Der \gls{dbt}-Term $\propto \lambda^{-3}$ dominiert für $\lambda < 10\,$cm.  
...
Die vollständige Bahngleichung in \gls{wg}-\gls{dbt}-Synthese lautet:
\begin{equation}
    \label{eq:r_wg_dbt}
...
Die Bahngleichung (\refeq{eq:weber_r_2_ordnung}) der \gls{wg} enthält einen unphysikalischen Term zweiter Ordnung $\alpha\phi^2$, der zu nicht-geschlossenen Bahnen führen würde.
Dieser Term wird jedoch durch das Quantenpotential der \gls{dbt} exakt kompensiert. Die Herleitung dieser Kompensation:
...
Die exakte Aufhebung des $\alpha\phi^2$-Terms demonstriert die konsistente Synthese von \gls{wg} und \gls{dbt} und unterstreicht die physikalische Validität des hybriden Ansatzes.
...
Der unphysikalische Term $\alpha \phi^{2}$ in der \gls{wg}-Bahngleichung (Gl. \refeq{eq:weber_r_2_ordnung}) würde zu einer spiralförmigen Abweichung führen, die nicht beobachtet wird.
...
\begin{itemize}
    ...
    \item \textbf{Experimentelle Konsequenzen:}\\Selbst moderne Tests der \gls{art} (z. B. LISA) sind nicht empfindlich genug, um solche Korrekturen zu messen. Die \gls{wg}-\gls{dbt}-Synthese ist somit in 1. Ordnung ausreichend genau.
\end{itemize}
...
Die exponentielle Wellenfunktion ist eine natürliche Näherung für gebundene Zustände, und die Kompensation des $\alpha \phi^{2}$-Terms demonstriert die Selbstkonsistenz der \gls{wg}-\gls{dbt}-Synthese.
...
Die Winkelgeschwindigkeit $\omega(\phi) = d\phi/dt$ folgt aus der \gls{wg}-Bahngleichung:
...
\begin{itemize}
\item Die \gls{wg} führt zu einer \textbf{modulierten Winkelgeschwindigkeit}, die von $\phi$ abhängt.
\item Im Perihel ($\phi = 0$) ist $\omega$ maximal, im Aphel ($\phi = \pi/\kappa$) minimal.
\item Die \gls{art} liefert eine äquivalente, aber strukturell unterschiedliche Vorhersage.
\end{itemize}
...
wobei $r(\phi) = \frac{a(1-e^2)}{1 + e \cos(\kappa \phi)}$ die \gls{wg}-Bahn beschreibt.  

\subsection*{Konsequenzen}  
\begin{itemize}  
\item \textbf{Mikroskopische Systeme}: Die \gls{dbt}-Korrektur $\propto \lambda^{-3}$ ist für Elektronen in Atomen messbar ($\Delta \omega/\omega \sim 10^{-4}$).  
\item \textbf{Planetenbahnen}: Der Effekt verschwindet ($\lambda \gg r$), aber die WG-Korrektur $\propto c^{-2}$ bleibt.  
\item \textbf{Unterschied zur \gls{art}}: Die \gls{dbt} führt zu einer \textbf{abstoßenden} Komponente, die Singularitäten verhindert.  
\end{itemize}
