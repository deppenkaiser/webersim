...
\section{Einleitung}
Dieses Kapitel beschreibt die numerische Umsetzung der \gls{iwt}. Da die Theorie fundamental diskret formuliert ist, lässt sie sich direkt als algorithmische Simulation
implementieren. Die numerischen Methoden dienen drei Hauptzwecken:
\begin{enumerate}
...
Großes $N$, fraktales Netz & \gls{art}-Geometrie & Teilweise bestätigt \\
...
\begin{itemize}
    \item \textbf{Ziel}: \gls{cmb}-Temperatur aus thermischem Gleichgewicht
    \item \textbf{Setup}: Großskaliges fraktales Netz mit $D = 2.71$
    \item \textbf{Ergebnis}: $T_{\text{\gls{cmb}}} \approx 2.7\,\text{K}$ ohne Urknall
...
Die numerische Simulation der \gls{iwt} zeigt:
...
\end{itemize}
Die numerischen Methoden bilden damit eine essentielle Brücke zwischen der theoretischen Formulierung der \gls{iwt} und ihren experimentell überprüfbaren Konsequenzen.
