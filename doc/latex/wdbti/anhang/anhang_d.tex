...
\section{Einleitung}
Dieser Anhang präsentiert ausführliche, mathematisch vollständige Rechnungen zu zentralen Beispielen der \gls{iwt}. Die Beispiele dienen drei Zwecken:
\begin{enumerate}
...
\end{enumerate}
Alle Berechnungen basieren auf den in Teil~I entwickelten diskreten Gleichungen und zeigen sowohl die diskrete Fundamentalebene als auch die emergenten kontinuierlichen
Grenzfälle.
...
\section{Beispiel 3: Kepler-Problem mit \gls{wg}}
...
\[
\vec{F}_{\text{\gls{wg}}} = -G \frac{Mm}{r^2} \left(1 - \frac{\dot{r}^2}{c^2} + \beta \frac{r\ddot{r}}{c^2}\right) \hat{r}
\]
...
\subsection{\gls{wg} für Photonen}
...
\hline
\textbf{Beispiel} & \textbf{\gls{iwt}-Lösung} & \textbf{Übereinstimmung mit etablierter Physik} \\
\hline
...
\hline
Plasmawellen & Dispersionsrelation aus \gls{wed} & Identisch mit \gls{mt} \\
\hline
...
\begin{enumerate}
    \item Die \gls{iwt} reproduziert alle erfolgreichen Vorhersagen etablierter Theorien
    \item Sie bietet natürliche Erklärungen für Phänomene, die in Standardtheorien ad-hoc postuliert werden müssen
    \item Sie macht spezifische zusätzliche Vorhersagen (frequenzabhängige Lichtablenkung, fraktale \gls{cmb}-Struktur)
    \item Alle Berechnungen basieren auf einem einheitlichen informationsbasierten Formalismus
\end{enumerate}

Die Beispiele demonstrieren damit die Konsistenz, Vollständigkeit und Anwendbarkeit der \gls{iwt} über alle Skalen hinweg – von der Quantenphysik bis zur Kosmologie.
