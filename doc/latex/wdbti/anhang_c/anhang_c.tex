...
Das Kepler-Problem zeigt, wie Gravitation in der \gls{wdbt} aus Informationsgradienten entsteht. Die zugrunde liegende Kraft ist die \gls{wg}:
\[
F_{\text{WG}}
=
-\,G\frac{M m}{r^2}
...
\right).
\]
Dies ist exakt dieselbe Struktur wie in der \gls{art}, jedoch ohne Raumzeitkrümmung.
...
\begin{itemize}
    ...
    \item Die \gls{wg} reproduziert die \gls{art}-Ergebnisse im schwachen Feld.
\end{itemize}

\section{Lichtablenkung als Modus der Informationsgeometrie}
\label{app:licht}

Für Photonen gilt in der \gls{wg}:
\[
\beta = 1.
\]
...
\frac{3GM}{c^2}u^2.
\]
Dies ist die Weber-Analogie der \gls{art}-Photonengleichung.
...
\frac{4GM}{c^2 b}.
\]
Dies ist exakt der \gls{art}-Wert.
...
\begin{itemize}
    ...
    \item Die Lichtablenkung stimmt exakt mit der \gls{art} überein.
