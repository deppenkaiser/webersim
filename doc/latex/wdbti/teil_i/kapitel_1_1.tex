...
\]
wird in dieser Arbeit als gegebene Größe verwendet, deren vollständige Herleitung aus den IWT-Axiomen ein offenes Forschungsprogramm bleibt. Sie charakterisiert die \textbf{Skalenstruktur des fundamentalen Informationsnetzes} und ist damit keine Raumdimension, sondern ein Maß für die Verzweigungskomplexität der Informationskopplungen.
...
Die IWT hebt diesen Gedanken auf eine fundamentale Ebene: Sie interpretiert alle physikalischen Größen als Informationsfunktionale und zeigt, dass \textbf{Informationserhaltung} die eigentliche Grundlage der Energieerhaltung ist.
...
\section{Aufbau dieses Buches (Teil I)}
\label{sec:aufbau}

Teil I entwickelt die IWT in ihrer fundamentalen diskreten Form:

\begin{enumerate}
    \item \textbf{Kapitel 1}: Einleitung und Motivation.
    \item \textbf{Kapitel 2}: Das diskrete Informationsfeld \( I_k^{(n)} \) und seine Erhaltung.
    \item \textbf{Kapitel 3}: Diskrete Weber-Dynamik als lokaler Grenzfall.
    \item \textbf{Kapitel 4}: Diskrete Informationsgeometrie und emergente Metrik \( g_{ij}^{(n)} \).
    \item \textbf{Kapitel 5}: Globale diskrete Dynamik (Bohm-Potential).
    \item \textbf{Kapitel 6}: Das diskrete Informationsnetz: Knoten, Kopplungen, Update-Regeln.
\end{enumerate}

Teil II wird zeigen, wie aus dieser diskreten Struktur die bekannte kontinuierliche Physik emergiert.

\section{Zusammenfassung}
\label{sec:zusammenfassung-einleitung}

Die Einleitung hat gezeigt:
\begin{itemize}
    \item Die IWT ist eine \textbf{fundamental diskrete Theorie} mit Information als Grundgröße.
    \item Sie löst konzeptionelle Probleme der modernen Physik durch explizite Diskretisierung.
    \item Die Weber-Kraft ist in diskreter Form \textbf{nicht-zirkulär und rekursiv}.
    \item Raum und Zeit sind emergente Eigenschaften des Informationsnetzes.
    \item Die Theorie ist empirisch verankert und macht testbare Vorhersagen.
\end{itemize}

Damit ist der Rahmen für die formale Entwicklung der diskreten Informations-Weber-Theorie gesetzt.
