...
Die Informations-Weber-Theorie basiert auf einem diskreten Informationsfeld \(I_k^{(n)}\), dessen Dynamik durch ein diskretes Informations-Lagrange-Funktional beschrieben
...
Da die Metrik \(g_{kl}^{(n)}\) nicht vorgegeben ist, sondern emergent aus der Kopplungsstruktur \(K_{kl}^{(n)}\), folgt ihre Dynamik aus der Variation des diskreten Funktionals:
\[
...
Damit ergibt sich die fundamentale dynamische Gleichung der IWT in diskreter Form:
\[
\boxed{
g_{kl}^{(n+1)} = g_{kl}^{(n)} + T \cdot \left[
\Delta_{k} I^{(n)} \Delta_{l} I^{(n)}
