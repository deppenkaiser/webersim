...
lassen sich die Anfangsbedingungen (Startwerte der Simulation) bestimmen. Anschließend werden die Störungen $\delta\vec{r}$, $\delta\vec{v}$,
$\delta\vec{\omega}$ berechnet, welche sich aus den Positionen und Geschwindigkeiten ergeben, und den jeweiligen Startwerten hinzugefügt.

An dieser Stelle ensteht die erste Ungenauigkeit der Simulation. Die Störungen der Himmelskörper werden von ungestörten
Positionen aus berechnet. Richtig wäre es, gestörte Ausgangswerte zur Berechnung der Störung eines bestimmten Himmelskörpers zu verwenden.
Das Problem hierbei wäre eine Rekursion, die erst dann endet, wenn die Störungen gegen einen bestimmten Wert konvergieren.
...
Für die Berechnung der Planeten wurden deren Monde (falls vorhanden), als zusätzliche Massen der Planetenmasse hinzugefügt.
...
berechnet werden. Die Integration findet also nicht über die Ortsvektoren und den Kräften statt, sonern über den Winkel; hierdurch
